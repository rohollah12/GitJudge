...
const exampleIssue =
  'https://github.com/microsoft/vscode-pull-request-github/issues/4508';
const examplePr =
...
export default function Page() {
  ...
  const canAnalyze = useMemo(() => {
    return Boolean(issueUrl.trim() && prUrl.trim() && !loading);
  }, [issueUrl, prUrl, loading]);
    ...
    <main style={pageStyle}>
      ...
      {result ? (
        <section style={resultGridStyle}>
          ...
          <div style={wideCardStyle}>
            <div style={sectionLabelStyle}>Issue</div>
            <p style={bodyTextStyle}>
              <strong>{meta.issue?.title ?? '—'}</strong>
              <br />
              {meta.issue?.body ?? 'No issue body returned.'}
            </p>
          </div>

          <div style={wideCardStyle}>
            <div style={sectionLabelStyle}>Pull request</div>
            <p style={bodyTextStyle}>
              <strong>{meta.pull_request?.title ?? '—'}</strong>
              <br />
              {meta.pull_request?.body ?? 'No PR body returned.'}
            </p>
          </div>
...
const buttonStyle = (enabled: boolean): CSSProperties => ({
  ...
  background: enabled
    ...
    : 'rgba(71, 85, 105, 0.9)',
  boxShadow: enabled ? '0 18px 30px rgba(59, 130, 246, 0.22)' : 'none',
});
...
const decisionStyle = (decision?: string): CSSProperties => ({
  fontSize: 30,
  fontWeight: 900,
  color:
    decision === 'PASS'
